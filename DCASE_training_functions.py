...
class LR_WarmRestartNew(keras.callbacks.Callback):
    def __init__(self, nbatch, initial_lr, min_lr, epochs_restart, Tmult):
        super(LR_WarmRestart, self).__init__()
        self.initial_lr = initial_lr
        self.min_lr = min_lr
        self.epochs_restart = epochs_restart
        self.nbatch = nbatch
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if logs is None:
            logs = {}
        # Adjust the current epoch number and Tmult
        self.currentEP = epoch
        if np.isin(epoch, self.epochs_restart):
            self.startEP = epoch
            self.Tmult *= 2
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        lr = self.model.optimizer.learning_rate.numpy()
        print(f"\nEpoch {epoch}: LearningRate: {lr:.6f}")
...
class MixupGenerator:
    def __init__(
        self, X_train, y_train, batch_size=32, alpha=0.2, shuffle=True, crop_length=400
    ):  # datagen=None):
    ...
    def __data_generation(self, batch_ids):
        ...
        for j in range(X1.shape[0]):
            ...
            X1[j, :, 0 : self.NewLength, :] = X1[
                j, :, StartLoc1 : StartLoc1 + self.NewLength, :
            ]
            X2[j, :, 0 : self.NewLength, :] = X2[
                j, :, StartLoc2 : StartLoc2 + self.NewLength, :
            ]
